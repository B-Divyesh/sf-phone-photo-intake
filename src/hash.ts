...
export async function hashBlob(blob: Blob, onProgress?: (ratio: number) => void): Promise<string> {
  const hash = new Sha256();
  const step = 1024 * 1024;
  if (blob.size === 0) hash.update(new Uint8Array());
  for (let offset = 0; offset < blob.size; offset += step) {
    hash.update(new Uint8Array(await blob.slice(offset, Math.min(offset + step, blob.size)).arrayBuffer()));
    onProgress?.(Math.min(1, (offset + step) / blob.size));
...
export async function hashText(value: string): Promise<string> {
  return new Sha256().update(new TextEncoder().encode(value)).hex();
}
